...
static pthread_cond_t squatting_condition;
int allow_squatting;
int refCount;
...
void init(int numthreads) {
 ...
 if(allow_squatting)
 {
  rv = pthread_cond_init(&squatting_condition, NULL);
  assert(rv == 0);
  }
}
...
int search  (const char *string, size_t strlen, int32_t *ip4_address) {
  ...
  if (strlen == 0)
	...
	int acquired = pthread_mutex_lock(&mutex);
	assert(acquired == 0);
  ...
  int released = pthread_mutex_unlock(&mutex);
  assert(released == 0);
...
int insert (const char *string, size_t strlen, int32_t ip4_address) {
  ...
  if(allow_squatting && !rv)
  { 
    assert(rv == 0);
   
	// loop until this thread is able to register
	// the desired key
    while(!rv)
	{
		assert(rv == 0);
		printf("Waiting on: %s\n", string);
		// wait until a node has been deleted
		refCount++;
    	int waitSuccessful = pthread_cond_wait(&squatting_condition, &mutex);
		assert(waitSuccessful == 0);
		refCount--;
...
int delete  (const char *string, size_t strlen) {
 ...
 if(allow_squatting)
  {
    // send a signal that a node has been deleted so 
	// the squatters can try to register it
	if(rv) // only send signal if delete successful
	{
	  // wake all threads, any one may be waiting for this particular node
      int signalSent = pthread_cond_signal(&squatting_condition);
	  assert(signalSent == 0);
	}
